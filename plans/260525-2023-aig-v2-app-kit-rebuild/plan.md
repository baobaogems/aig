...
| 03 | [phase-03-payment-page-dual-path.md](phase-03-payment-page-dual-path.md) | ✅ DONE (e2e green 30/05) | 26-30/05 | SHIPPED via standard CCTP TokenMessenger. Source: **Ethereum Sepolia**. New: `cctp-abi.ts`, `payment-flow-v2.ts` (2-sig flow), inline `PaymentPageV2`. **e2e smoke ✅** 30/05: Sepolia burn `0x9a620cf2…f5882a8b424094f4d26dd51bbfdd87d3a2070b34aae4edffa16` → Iris v2 attest → Arc mint `0xc0b4cca9…7963c92d43afaada02c65e093567e6f374d04e667acdf641eb61` (block 44698950, merchant +0.996383 USDC). Required follow-up fixes (all landed): Fast Transfer (`maxFee=amountWei/1000n`>0), `pollAttestationV2` against CCTPv2 Iris endpoint, EIP-1559 fee+gas pin on Sepolia writes, SSE stream-close guard. See `status_AIG.json` entries 2026-05-26 through 2026-05-30. |
| 04 | [phase-04-flip-default-to-v2.md](phase-04-flip-default-to-v2.md) | doc ready (user step pending) | 26-29/05 | REWRITTEN to match Phase 03 ship. Vercel dashboard: ADD 8 env vars (6 NEXT_PUBLIC_* + BRIDGE_BACKEND + ETHEREUM_SEPOLIA_RPC_URL + verify CCTP_MESSAGE_TRANSMITTER_ARC), KEEP v1 vars for rollback, DROP KIT_KEY from plan. Pre-flight: local Phase 03 smoke must pass first. Rollback = 2 env flips + redeploy (~2 min). |
...
| 06 | [phase-06-cleanup-v1.md](phase-06-cleanup-v1.md) | ✅ FULL DONE (31/05) | 30-31/05 | Partial (30/05): App Kit dead code (commit 836b584). Full (31/05): user-overrode 48h smoke gate to rip out v1 stack entirely — deleted `contracts/`, `mock-bridge.ts`, v1 helpers in `cctp.ts`/`agent.ts`, `/api/agent/quote`, v1 `PaymentPage`, `FeeBreakdownCard`, `bscTestnet` from wagmi; collapsed `BRIDGE_BACKEND` dispatch in execute route. `tsc` + `next build` green (7 routes). Rollback now via git revert; v1.0 tag preserves prior codebase. |
