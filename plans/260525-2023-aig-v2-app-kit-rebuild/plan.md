...
| 03 | [phase-03-payment-page-dual-path.md](phase-03-payment-page-dual-path.md) | ✅ code landed (smoke pending) | 26/05 | SHIPPED via standard CCTP TokenMessenger (not the mystery `kitContracts.bridge` — see ADR step 0). Source chain: **Ethereum Sepolia** (BSC isn't a CCTPv2 chain). New: `cctp-abi.ts`, `payment-flow-v2.ts` hook (2-sig flow: USDC.approve + TokenMessenger.depositForBurn(domain 26)), inline `PaymentPageV2` component. cctp.ts parameterized via `SourceChainConfig` (back-compat default = V1_BSC_SOURCE). execute route branches on `BRIDGE_BACKEND=v2`. Build green. Real Arc Testnet smoke = user step (needs funded Sepolia wallet + USDC). |
