...
| 06 | [phase-06-cleanup-v1.md](phase-06-cleanup-v1.md) | 🟡 PARTIAL (30/05) | 30/05 — TBD | **App Kit dead code shipped 30/05** (`appkit.server.ts`, `/api/dev/appkit-ping`, `@circle-fin/*` deps, unused `V2_ETH_SEPOLIA_SOURCE`, `KIT_KEY` env). Build green. **v1 stack deletes deferred** until Vercel flip + 48h smoke: `/contracts`, v1 helpers in `cctp.ts`, `mock-bridge.ts`, admin-relay scripts, v1 PaymentPage, `/api/agent/quote`, README banner. Post-ADR plan corrections in phase file. |
