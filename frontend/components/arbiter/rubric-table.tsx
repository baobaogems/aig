    ...
    <div>
      ...
      <ul className="mt-4 m-0 p-0 list-none space-y-3">
        {items.map((r) => (
          ...
          >
            <span className="text-[13px] leading-relaxed text-[var(--a-text)]">{r.criterion}</span>
            <span className="a-tnum shrink-0 font-[family-name:var(--font-jetbrains-mono)] text-[12px] text-[var(--a-subtle)] font-bold">
              {r.weight}%
            </span>
          </li>
        ))}
      ...
      <p className="mt-4 m-0 text-[11px] text-[var(--a-subtle)]">
        Tổng trọng số: <span className="a-tnum font-bold">{total}%</span>
        {total !== 100 && (
