...
export function LandingPilotMetrics() {
  return (
    // Cùng nền sáng với khối safety ngay trên nó (đổi 22/09/2026, trước là obsidian). Một
    // đường kẻ mảnh đánh dấu chỗ chuyển từ lời tuyên bố sang bằng chứng — giữ mảng liền
    // mạch, không cắt rời bằng khoảng trống hay màu nền khác.
    <section
      id="evidence"
      className="border-t border-[var(--color-border-light)] bg-[var(--color-surface-light)] py-24"
    >
          ...
          {CASES.map((c) => (
            <Reveal key={c.label}>
              <GlassPanel tone="light" interactive className="h-full p-6">
                ...
                <p className="tnum mt-1 text-sm text-[var(--color-ink-muted)]">{c.amount}</p>
