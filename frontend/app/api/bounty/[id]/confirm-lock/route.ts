...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (isDryRun())
      return Response.json({ error: "DRY_RUN — there are no on-chain funds to confirm" }, { status: 409 });

    const detail = await getBountyDetail(id);
    if (detail.bounty.status === "OPEN")
      return Response.json({ ok: true, escrow_tx: detail.bounty.escrow_tx, note: "already opened earlier" });
    ...
    if (!onChain || onChain.poster === "0x0000000000000000000000000000000000000000")
      return Response.json({ error: "no escrow found on chain — has the transaction been mined?" }, { status: 409 });
    ...
    if (!sameAddress(onChain.poster, caller))
      return Response.json(
        { error: `the escrow was locked by a different wallet (${onChain.poster}), not the one signed in` },
        { status: 409 },
      );
    const expected = usdcToUnits(Number(detail.bounty.amount_usdc));
    if (onChain.amount !== expected)
      return Response.json(
        { error: `the on-chain amount (${onChain.amount}) differs from the approved amount (${expected})` },
        { status: 409 },
      );
    ...
    if (Number(onChain.deadline) !== expectedDeadline)
      return Response.json(
        { error: `the on-chain deadline differs from the approved deadline` },
        { status: 409 },
      );

    await freezeRubric(id, txHash);
    return Response.json({ ok: true, escrow_tx: txHash ?? null, note: "USDC is in escrow, rubric frozen, bounty open" });
  } catch (err) {
